	...
	private Target Test =>
		_ =>
			_.DependsOn(Clean)
				...
				{
					DotNetTest(_ =>
						_.AddProcessAdditionalArguments("--solution", Solution.Path).SetConfiguration(Configuration).SetOutput("detailed")
					);
				});
				...
				.Executes(() =>
				{
					DotNetPack(_ =>
						_.SetConfiguration(Configuration)
							...
							.SetProperty("PublicRelease", PublicReleaseStr)
					);
				})
				.Executes(() =>
				{
		...
		_ =>
			_.DependsOn(Pack)
				...
				.Executes(() =>
				{
					...
					{
						DotNetNuGetPush(p => p.SetApiKey(NuGetApiKey).SetSkipDuplicate(true).SetSource(NuGetSource).SetTargetPath(nupkg));
					}
				});
}
